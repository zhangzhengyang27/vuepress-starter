...
![源代码与编译转换后代码之间的映射](https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/sourcemap-mapping.png)目前很多第三方库在发布的文件中都会同时提供一个 .map 后缀的 Source Map 文件。例如 jQuery。我们可以打开它的 Source Map 文件看一下，如下图所示：

![jQuery 的 Source Map](https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/jquery-sourcemap.png)
...
![jQuery Source Map 的 Mappings](https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/jquery-mappings.png)
...
![引入 jQuery 的 Source Map 文件](https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/import-sourcemap-file.png)
...
![根据 Source Map 解析出来的源代码](https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/resolve-source.png)

我们还可以添加一个断点，然后刷新页面，进行单步调试，此时调试过程中使用的就是源代码而不是压缩过后的代码，具体效果如下图所示：

![Source Map 的效果](https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/sourcemap-effect.png)
...
![Webpack 输出 Source Map 文件](https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/webpack-sourcemap.png)
...
![JavaScript 中的 eval 函数](https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/evel-code.png)

其实我们可以通过 `sourceURL` 来声明这段代码所属文件路径，接下来我们再来尝试在执行的 JavaScript 字符串中添加一个 `sourceURL` 的声明，具体操作如下：

![eval 函数的 sourceURL](https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/evel-sourceurl.png)
...
![eval 模式 Source Map](https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/webpack-eval-mode.png)

那此时如果我们回到浏览器运行这里的 bundle.js，一旦出现错误，浏览器的控制台就可以定位到具体是哪个模块中的代码，具体效果如下：

![eval 模式下的效果](https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/webpack-eval-effect.png)



但是当你点击控制台中的文件名打开这个文件后，看到的却是打包后的模块代码，而并非我们真正的源代码，具体如下：

![eval 模式映射出的源代码](https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/webpack-eval-source.png)
...
![多任务打包输出](https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/multi-output.png)
...
![不同模式下的打包结果](https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/all-sourcemap-modes.png)



然后我们通过 serve 把结果运行起来，打开浏览器，此时我们能够在页面中看到每一个使用不同模式 Source Map 的 HTML 文件，具体如下图：

![不同模式下的打包结果入口](https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/all-sourcemap-entries.png)那如果刚刚没有把 JS 文件输出到单独目录中，这里的文件就会非常多，导致 HTML 文件寻找起来特别麻烦。
...
![eval 模式](https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/eval-mode.png)
...
<video src="https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/eval-sourcemap.mp4"></video>



紧接着我们再来看一个叫作 cheap-eval-source-map 的模式。根据这个模式的名字就能推断出一些信息，它就是在 eval-source-map 基础上添加了一个 cheap，也就是便宜的，或者叫廉价的。用计算机行业的常用说法，就是阉割版的 eval-source-map，因为它虽然也生成了 Source Map 文件，但是这种模式下的 Source Map 只能定位到行，而定位不到列，所以在效果上差了一点点，但是构建速度会提升很多。

<video src="https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/cheap-eval-sourcemap.mp4"></video>
...
![cheap-eval-source-map 对比 cheap-module-eval-source-map](https://interview-aliyun.oss-cn-beijing.aliyuncs.com/zce/module-vs-without.png)
