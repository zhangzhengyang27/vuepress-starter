...
这是因为在这个过程中 Webpack 为了实现这些特性，会自动往打包结果中添加一些内容。例如我们之前用到的 [Source Map](https://blog.zce.me/2020/04/best-practices-of-webpack-sourcemap/) 和 [HMR](https://blog.zce.me/2020/04/webpack-hot-module-replacement/)，它们都会在输出结果中添加额外代码来实现各自的功能。
...
我们回到命令行，这里我们以生产模式运行打包。那按照之前的了解，生产模式下会自动压缩输出的结果，我们可以打开打包生成的 JS 文件。

然后我们再打开输出的样式文件。这里我们发现 JavaScript 文件正常被压缩了，而样式文件并没有被压缩。
...
那此时我们再次回到命令行运行打包。打包完成过后，我们的样式文件就会以压缩格式输出了。
